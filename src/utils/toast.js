...
function ToastCard({ t, type, title, description }) {
  return React.createElement(
    motion.div,
    {
      ...
      className: 'pointer-events-auto flex max-w-[360px] items-start gap-3 rounded-2xl border border-[color:var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow)] backdrop-blur-xl',
    },
    React.createElement('div', { className: 'mt-0.5 flex-shrink-0' }, iconMap[type]),
    React.createElement(
      'div',
      { className: 'min-w-0 flex-1' },
      React.createElement('p', { className: 'text-sm font-semibold text-[var(--text)]' }, title),
      description ? React.createElement('p', { className: 'mt-1 whitespace-pre-line text-sm leading-6 text-[var(--text-secondary)]' }, description) : null,
    ),
    React.createElement(
      'button',
      {
        ...
        className: 'rounded-full p-1 text-[var(--text-secondary)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--text)]',
